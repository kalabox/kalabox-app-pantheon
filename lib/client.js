  ...
  _.map(sites, function(val, key) {
    if (!_.isEmpty(val)) {
      choices.push({
        ...
        value: val.site.name,
        id: val.site.name,
        uuid: val.site.id || key,
        php: val.site.php_version,
        upstream: val.site.upstream,
        framework: val.site.framework,
        drush: val.site.drush_version || 7
      });
    }
  });
...
/*
 * Get full list of dashboard sites
 * Only meant for use in this module. Dont call directly.
 *
 * sites/USERID/sites
 */
Client.prototype.__getDashboardSites = function(session) {
  return this.__request(
    'get',
 ...
 * Only meant for use in this module. Dont call directly.
 *
 * users/USERID/memberships/organizations
 * EACH organizations/ORGID/memberships/sites
 */
Client.prototype.__getOrgSites = function(session) {

  // Save for later
  var self = this;

  // Get all our org memberships
  return self.__request(
    'get',
    ['users', session.user_uuid, 'memberships', 'organizations'],
    {headers: self.__getAuthHeaders(session)}
  )

  // Grab all our sites from each org
  .map(function(org) {

    // If the role make sense
    if (org.role !== 'unprivileged') {
      return self.__request(
        'get',
        ['organizations', org.id, 'memberships', 'sites'],
        {headers: self.__getAuthHeaders(session)}
      )
      .map(function(site) {
        var target = {};
...
Client.prototype.__getSites = function(session) {
  ...
  // Start a collector
  // @todo: probably a more promisy/lodashy way to do this]
  var targets = [];

  // Get our dashboard sites
  return self.__getDashboardSites(session)
  .then(function(sites) {
    targets = targets.concat(parseSites(sites));
    return targets;
  })

  // Get our org sites
  .then(function() {
    return self.__getOrgSites(session)
    .then(function(sites) {
      targets = targets.concat(parseSites(sites));
      return targets;
    });
  ...
  // Unique it
  .then(function(sites) {
    return _.sortBy(_.uniq(sites), 'name');
  });
};
...
Client.prototype.getEnvs = function(token, site) {
  ...
  .then(function(sites) {
    var uuid = _.find(sites, function(s) {
      return s.name === site;
    });
    return uuid.uuid;
  })
