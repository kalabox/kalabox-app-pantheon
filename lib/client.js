...
Client.prototype.getEnvs = function(token, site) {
  ...
  .then(function(dashSites) {

    // define the target UUID
    var uuid;

    // Try to get target site from dashboard
    uuid = _.findKey(dashSites, function(s) {
      return s.information.name === site;
    });

    // We've got a good UUID from our dashboard so lets use that
    if (!_.isEmpty(uuid)) {
      return uuid;
    }

    // Let's look through our ORGS for a valid UUID
    else {

      // Get org sites
      return self.__getOrgSites(session)

      // Look for a UUID that matches
      .then(function(orgSites) {
        var get = _.find(orgSites, function(s) {
          return s.information.name === site;
        });
        return get.information.id;
      });
    }
