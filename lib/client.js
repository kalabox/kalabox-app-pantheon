...
Client.prototype.__getOrgSites = function(session) {
  ...
  .map(function(org) {
    var name = org.organization.profile.machine_name || 'unknown';
    self.log.debug(format('Gettings sites from %s', name));
    return self.__request(
      'get',
...
Client.prototype.getEnvs = function(token, site) {
  ...
  .then(function(uuid) {
    if (_.isEmpty(uuid)) {
      self.log.debug(format('Site %s not found in dashboard.', site));
      self.log.debug(format('Looking in orgs.'));
      return self.__getOrgSites(session)
      .then(function(sites) {
        var get = _.find(sites, function(value) {
          return value.information.name === site;
        });
