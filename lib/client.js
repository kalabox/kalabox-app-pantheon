...
Client.prototype.__setSession = function(session) {
  ...
  if (session) {
    // Save in run cache
    // @todo: get the email in there somehow?
    this.session = {
      session: session.session,
      session_expire_time: session.expires_at,
...
Client.prototype.__getSessionHeaders = function(session) {
  ...
  if (session === undefined) {
    // @todo: if this is false/undefined we need to tell the user to login
    // btu maybe that happens actually in __getSession?
...
Client.prototype.getSites = function(session) {
  ...
  if (session === undefined) {
    // @todo: if this is false/undefined we need to tell the user to login
    // btu maybe that happens actually in __getSession?
    session = this.__getSession(session);
  }
 ...
 * Get full list of sites
 */
Client.prototype.getEnvironments = function(session, uuid) {
